...
export async function addMessageReaction(message: Message, emoji: string) {
  await invoke('add_message_reaction', { chatId: message.chat_id, messageId: message.id, emoji });
}

export async function removeMessageReaction(message: Message, emoji: string) {
  await invoke('remove_message_reaction', { chatId: message.chat_id, messageId: message.id, emoji });
}
